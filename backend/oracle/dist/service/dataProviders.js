export async function fetchExchangeRate(base, quote) {
    const API_URL = process.env.EXCHANGE_RATE_API_URL || 'https://api.coingecko.com/api/v3';
    try {
        const response = await fetch(`${API_URL}/simple/price?ids=ethereum&vs_currencies=idr&include_last_updated_at=true`, {
            headers: {
